...
// 日本の主要な川の代表的な観測地点の緯度経度
export const riverCoordinates: RiverCoordinates = {
  // 長野県
...
/**
 * 川のリストから緯度経度が設定されている川のみを返す
 */
export function getRiversWithCoordinates(): string[] {
  return Object.keys(riverCoordinates);
}

/**
 * 都道府県内の川の緯度経度を取得
 */
export function getCoordinatesByPrefecture(prefecture: string): RiverCoordinates {
  const result: RiverCoordinates = {};
  
  for (const [riverName, coords] of Object.entries(riverCoordinates)) {
    if (coords.prefecture === prefecture) {
      result[riverName] = coords;
    }
  }
